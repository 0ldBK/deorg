...
if(isset($_POST['uid'])) {
    ...
    } elseif(isset($_POST['timeleft']) && $uid > 1) {
    	//Y-m-d
    	$t = explode("-", $_POST['timeleft']);
    	$timeleft = mktime(23, 59, 59, $t[1], $t[2], $t[0]);
    	$status = $timeleft > time() ? 0 : 3;

    	if($db->sql_query("UPDATE vip SET `vip` = '$timeleft', `access` = '$status'  WHERE uid = '$uid'")) {
...
$page = isset($_GET['page']) && ceil($_GET['page']) > 0 ? ceil($_GET['page']) : 1;
$limit = 100;
$offset = ($page-1) * $limit;
...
<table border=1 cellspacing=0 cellpadding=5 style='font-size: 12px; border: 1px solid #000040;'>
    <tr><th>Аккаунт</th><th>Platnum</th><th>Gold</th><th>Silver</th><th>Прочие</th><th>Итого</th></tr>
    <tr><td>Всего</td><td>".$stat['platinum']."</td><td>".$stat['gold']."</td><td>".$stat['silver']."</td><td>".$stat['other']."</td><td>".$stat['all']."</td></tr>
    <tr><td>Онлайн</td><td>".$stat['platinum2']."</td><td>".$stat['gold2']."</td><td>".$stat['silver2']."</td><td>".$stat['other2']."</td><td>".$stat['all2']."</td></tr>
</table>
<br>
